...
assembled, compiled and translated into documentation.
...
------
emulation: add.flash
	qemu-system-arm -M connex -pflash add.flash -nographic -serial /dev/null
flash.bin: add.bin
	dd if=/dev/zero of=flash.bin bs=4K count=4K
	dd if=$< of=flash.bin bs=4K conv=notrunc
add.bin: add.elf
	arm-none-eabi-objcopy -O binary add.elf add.bin
add.elf: add.o	
...
Very useful make tip: variables. First of all, you can use two
special variables: $@ and @<.

$@ variable represents left side of Makefile rule
...
.PHONY: emu
emu: $(APP).flash
	qemu-system-arm -M connex -pflash $< -nographic -serial /dev/null
------
