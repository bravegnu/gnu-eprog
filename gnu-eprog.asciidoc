...
make (using simple .rc SHELL of .bat ch file) every tiny change in source file
...
Then, we replaced all file parameters in assembler/linkers calls with 
...
Wow, we compiler other program without any change in Makefile, all file names
was renamed automatically, we just get separate flash image and running Qemu 
in one command.
...
Some vars widely used in makefiles of Linux program sources. You can use this
makefiles as samples for lot of make tricks, not covered by this tutorial.
...
`BUILD`:: triplet for developer computer, something like x86_64_linux_gnu. 
...
Finaly, let's do some strange thing: define universal rules, which will 
...
Just define two variables and one rule:

------
S = file1.s file2.s ... file333.s
S += file334.s ... file100500.s
O = file1.o file2.o ... file333.o
O += file334.o ... file100500.o
$(O): $(S)
------
* `+=` adds values to variable

and metarules set will be automagically build your huge project.
